...
void read_tetmesh()
{
	...
	clock_t tic, toc; //Code timing variables
	double	x,y,z ;
	double  *Vol;
	...
	printf("This is the parallel(OpenMP) version of the code. There are %d threads available for parallel processing.\n",omp_get_max_threads() );
	...
	fclose(fp) ;
	toc = clock();
	printf("Time to Read Node Data: %f milliseconds\n", 1000*((double)(toc - tic) / CLOCKS_PER_SEC));
	...
	fclose(fp); //Close face data file
	toc=clock();
	printf("Time to Read Face Data: %f milliseconds\n", 1000*((double)(toc - tic) / CLOCKS_PER_SEC));
	...
	*/
	...
	toc=clock();
	printf("Time to Re-Map Face Data: %f milliseconds\n", 1000*((double)(toc - tic) / CLOCKS_PER_SEC));
	//Debugging
	/*
        for(i=0;i<nFaces;i++)
        { 
...
//*************************************************************
	...
	printf("What is output casename?  i.e. <casename>.surf:");
	scanf("%s",casename);
